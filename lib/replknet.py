...
if __name__ == '__main__':
    ...
    x = torch.randn(1, 320, 16, 16).cuda()
    # dpr = [x.item() for x in torch.linspace(0, 0.3, 1)]
    # dpr[sum(layers[:stage_idx]):sum(layers[:stage_idx + 1])]
    # model=RepLKBlock(320,512,31,5,0.3).cuda()
    origin_y = model(x)
